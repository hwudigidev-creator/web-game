...
**版本**: v0.9.6a（右下角顯示，由 package.json 自動注入）
...
| Lv.3 | 90° 扇形、5 傷害 |
| Lv.4 | 100° 扇形、6 傷害 |
| MAX | 3 向衝擊波（0°/120°/240°）、10 傷害、10 單位射程 |
...
| Lv.3 | 4 道光束、4 傷害 |
| Lv.4 | 5 道光束、5 傷害 |
| MAX | 3 道精準鎖定超粗射線、6 傷害、15 單位射程 |

- **MAX 額外能力「精準」**: 改為 3 條精準鎖定超粗射線，射程 15 單位

#### 靈魂統領（架構師）
- **顏色**: 金色 | **冷卻**: 10 秒 | **效果**: 30% HP 護盾（霸體）+ 反傷
...
| 技能名稱 | 顏色 | 效果 | Lv.0 | MAX | MAX 額外能力 |
|----------|------|------|------|-----|--------------|
| 鈦金屬賽博肝臟 | 銀灰色 | 每級 +10% HP，每 15 秒回血（每級 -1 秒） | 10% / 15s | 60% / 10s | 「不死」抵銷一次死亡，觸發暗影爆炸 |
| 精神同步率強化 | 暗橘色 | 每級 +10% 移速、-8% 冷卻 | 10% / 8% | 60% / 48% | 「迅捷」閃避機率 +0.2%/級 |
| 視網膜增強模組 | 暗紅色 | 每級 +0.5 單位拾取範圍 | 0.5 單位 | 3 單位 | 「洞察」暴擊率 +0.5%/級 |
| AI賦能強化 | 灰藍色 | 每級 +25% 攻擊、+15% 防禦 | 25% / 15% | 150% / 90% | 「超載」暴擊傷害 +0.5%/級 |
...
| 絕對邏輯防禦 | 靈魂統領（架構師）+ 精神同步率強化 | 持續 | 有護盾時在 2 單位距離產生輪鋸繞角色旋轉（滿盾 6 個，2 秒一圈，0.5 單位範圍），撞敵消耗 2% 護盾 |
...
| 零信任防禦協定 | 靈魂統領（架構師）+ AI 賦能強化 | 持續 | 展開 5 單位八角矩陣，持續光束掃描範圍內敵人，範圍內敵人移動速度減半 |
...
| 絕對邏輯防禦 | 護盾技能 + 移速強化 | 架構師的防禦邏輯化為實體輪鋸，護盾越強輪鋸越多 |
