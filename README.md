...
**版本**: v0.9.26a（右下角顯示，由 package.json 自動注入）
...
- **生成**: 每次 6 隻，間隔 = 3 秒 / (180 / 場上怪物數)，怪物越少生越快，最短 0.1 秒；每 5 級額外多生成 1 隻
- **50 級後強化**: 每 5 級怪物血量、傷害、經驗值 +15%
...
| 菁英史萊姆 | 暗紫色 | 15% | 1x | 3x | 每 5 級出現，HP = 基礎 × 等級，死亡掉落血球 |
...
遊戲開始前可選擇難度模式，影響怪物生成、經驗獲取和負面狀態抵抗：

| 模式 | 怪物倍率 | 經驗倍率 | 移速倍率 | HP 倍率 | 傷害倍率 | 負面狀態抵抗 |
|------|----------|----------|----------|---------|----------|--------------|
| **日常** | 1.0x | 1.0x | 0.8x | 1.0x | 1.0x | 0% |
| **噩夢** | 2.0x | 2.0x | 1.2x | 2.0x | 1.5x | 25% |
| **煉獄** | 3.0x | 3.0x | 1.5x | 2.5x | 2.0x | 50% |

### 負面狀態抵抗

怪物依難度有機率抵抗負面狀態（擊退、暈眩、燃燒等）：
- **日常**: 無抵抗，所有負面狀態 100% 生效
- **噩夢**: 25% 機率抵抗
- **煉獄**: 50% 機率抵抗

## 排行榜系統

遊戲結束後可提交成績至線上排行榜，支援多難度分頁：
...
| 等級 | 效果 | 升級說明 |
|------|------|----------|
| Lv.0 | +30% 經驗、+0.5 拾取範圍 | 敏銳的觀察力，視野範圍擴大了 |
| Lv.1 | +60% 經驗、+1 拾取範圍 | 敏銳的觀察力，視野範圍擴大了 |
| Lv.2 | +90% 經驗、+1.5 拾取範圍 | 敏銳的觀察力，視野範圍擴大了 |
| Lv.3 | +120% 經驗、+2 拾取範圍 | 敏銳的觀察力，視野範圍擴大了 |
| Lv.4 | +150% 經驗、+2.5 拾取範圍 | 敏銳的觀察力，視野範圍擴大了 |
| MAX | +180% 經驗、+3 拾取範圍 | 千錘百鍊的敏銳觀察，再沒什麼看不穿、看不清 |
...
- **等級系統**：與一般技能統一，從 Lv.0 開始（Lv.0 = 無技能等級加成）

#### 進階技能傷害加成
進階技能的等級會加成到對應的主動技能傷害上：

| 主動技能 | 對應進階技能 | 加成方式 |
|----------|--------------|----------|
| 靈魂渲染 | 燃燒的賽璐珞 + 次元向量疾劃 | 傷害單位 += 兩者等級總和 |
| 咒言幻象 | 技術美術大神 + 幻影迭代模式 | 傷害單位 += 兩者等級總和 |
| 疾光狙擊 | 完美像素審判 + 爆發的影視特效 | 傷害單位 += 兩者等級總和 |
| 靈魂統領 | 絕對邏輯防禦 + 零信任防禦協定 | 反傷單位 += 兩者等級總和 |

範例：技術美術大神 Lv.2 + 幻影迭代 Lv.10 = 咒言幻象傷害 +12 單位

#### 進階技能列表

| 技能名稱 | 組合需求 | 冷卻 | 效果 |
|----------|----------|------|------|
| 燃燒的賽璐珞 | 靈魂渲染 + 鈦金肝 | 2 秒 | -10HP 旋轉攻擊 7 單位，(10%+Lv%)燃燒 DOT |
| 技術美術大神 | 咒言幻象 + 視網膜 | 1 秒 | 5 單位內落雷，3 單位爆炸 + 暈眩 1 秒 |
| 絕對邏輯防禦 | 靈魂統領 + 同步率 | 持續 | 3+Lv/5 輪鋸(max8)，撞敵耗 1% 盾，盾覆蓋時射向敵人彈飛 |
| 完美像素審判 | 疾光狙擊 + 視網膜 | 3 秒 | 井字線 4 焦點各炸 1 次，3 單位 + 暈眩 1 秒 |
| 爆發的影視特效 | 疾光狙擊 + AI 強化 | 5 秒 | 2 秒 30 導彈追蹤，命中 Lv% 燃燒 DOT |
| 幻影迭代模式 | 咒言幻象 + AI 強化 | 8 秒 | 分身 max3，滿時每分身咒言圈 2 秒(50%傷害，觸發燃燒) |
| 零信任防禦協定 | 靈魂統領 + AI 強化 | 持續 | 8 光點 5 單位場，減速 50%，擊殺 Lv% 重置盾 CD |
| 次元向量疾劃 | 靈魂渲染 + AI 強化 | 0.5 秒 | 全螢幕直線斬擊，暴擊 3 倍，每級 1% 連鎖 |
...
| 絕對邏輯防禦 | 護盾技能 + 移速強化 | 架構師的防禦邏輯化為實體輪鋸，盾覆蓋時射向敵人彈飛 |
| 完美像素審判 | 光束攻擊 + 暴擊強化 | 導演的精準構圖，青綠色井字線定位爆破目標並暈眩敵人 |
| 爆發的影視特效 | 光束攻擊 + AI 強化 | 導演的華麗特效，追蹤導彈群飛射擊並燃燒 |
| 幻影迭代模式 | 圓形範圍 + AI 強化 | 先知的分身術，AI 複製體嘲諷，滿員時咒言圈追蹤傷害 |
| 零信任防禦協定 | 護盾技能 + AI 強化 | 架構師的安全協定，八角矩陣掃描減速，擊殺重置護盾 |
| 次元向量疾劃 | 扇形攻擊 + AI 強化 | 向量運算次元切割，全螢幕斬擊連鎖 |
...
| 項目 | 規格 |
|------|------|
| **輪鋸數量** | 基礎 3 個 + floor(技能等級 / 5)，最多 8 個 |
| **滿員等級** | 技能等級 25（3 + 25/5 = 8 個） |
| **護盾消耗** | 每撞擊敵人 1 次消耗 1% 護盾 |
| **消耗遞減** | 滿 8 個後，每升 1 級減少 0.1% 消耗（25 級 = 1%，35 級 = 0%） |
| **護盾回復** | 消耗率降至 0% 以下時，撞敵反而回復護盾 |
| **旋轉軌道** | 2 單位距離繞角色旋轉 |
| **旋轉週期** | 2 秒一圈 |
| **輪鋸範圍** | 0.5 單位 |
| **飛出機制** | 護盾覆蓋時輪鋸朝最近敵人飛出 |
| **彈跳追蹤** | 擊中後搜尋前進方向 ±45°（90度扇形）內最近敵人繼續飛行 |
| **飛行速度** | 每秒 15 單位，飛出 15 單位範圍消失 |
