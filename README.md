...
**版本**: v0.7.0a（右下角顯示，由 package.json 自動注入）
...
| 技術美術大神 | 咒言幻象（遊戲先知）+ AI 賦能強化 | 1 秒 | 在角色周圍 5 單位隨機地點射下光線，3 單位爆炸範圍，命中敵人癱瘓 0.5 秒，傷害 = (角色等級 + 技能等級) 單位 |
| 絕對邏輯防禦 | 靈魂統領（架構師）+ 精神同步率強化 | 持續 | 有護盾時在 1.5 單位距離產生 1 單位範圍輪鋸繞角色旋轉，護盾越高轉越快（最快 1 秒/圈），傷害 = (角色等級 + 技能等級) 單位 |
...
| 絕對邏輯防禦 | 護盾技能 + 移速強化 | 架構師的防禦邏輯化為實體輪鋸，護盾越強轉速越快 |
