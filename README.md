...
**版本**: v0.4.0a（右下角顯示，由 package.json 自動注入）
...
- **HP 條**: 網格式暗紅暗紫流動漸層，顯示 當前(+護盾)/最大 HP，損傷白色區塊 1 秒後靠攏
- **護盾條**: 金色至白金色流動漸層，覆蓋於 HP 條上層
- **經驗條**: 網格式藍紫流動漸層動畫效果
- **技能選擇面板**: 升級時彈出，A/S/D 選擇、重複按同一鍵確認（手機版隱藏按鍵提示）
...
- **視覺**: 金色至白金色流動漸層，覆蓋於 HP 條上層，受擊金色閃光
...
| 架構師 | 靈魂統領 | 金色 | 10 秒 | 護盾（最大 HP 30%，霸體）+ 反傷，每級 +1.5 反傷（Lv.0=1u, MAX=8.5u），護盾破碎回血 |
...
|----------|------|------|------|------------|
| 鈦金肝 | 銀灰色 | 每級 +10% HP，每 15 秒回復 1% 最大 HP（每級 -1 秒） | 10% / 15s | 60% / 10s |
...
| 技能名稱 | 額外能力 | 效果 | 每級成長 |
|----------|----------|------|----------|
| 靈魂渲染 | 穿透 | 攻擊後發射扇形波（外半部）向前移動 5 單位 | +0.4%/級 |
| 遊戲先知 | 爆發 | 擊殺時從該位置再發動機率 | +0.2%/級 |
| 超級導演 | 連鎖 | 擊殺時從該位置朝隨機目標再發射 | +0.2%/級 |
| 靈魂統領 | 堅守 | 護盾覆蓋時向外炸開 3 單位並回血 | +1%/級 |
| 鈦金肝 | 再生 | 額外回復量 | +0.1%/級 |
| 精神同步率強化 | 迅捷 | 閃避機率 | +0.1%/級 |
| 視網膜增強模組 | 洞察 | 暴擊率 | +0.1%/級 |
| AI賦能強化 | 超載 | 暴擊傷害加成 | +0.2%/級 |
