...
**版本**: v0.8.6a（右下角顯示，由 package.json 自動注入）
...
| 技能名稱 | 組合需求 | 冷卻 | 效果 |
|----------|----------|------|------|
| 燃燒的賽璐珞 | 靈魂渲染（動畫大師）+ 鈦金屬賽博肝臟 | 2 秒 | 消耗 10 HP，以角色為中心旋轉一圈 30° 扇形攻擊，傷害 = (角色等級 + 技能等級) 單位 |
| 技術美術大神 | 咒言幻象（遊戲先知）+ AI 賦能強化 | 1 秒 | 在角色周圍 5 單位隨機地點射下光線，3 單位爆炸範圍，命中敵人癱瘓 1 秒，傷害 = (角色等級 + 技能等級) 單位 |
| 絕對邏輯防禦 | 靈魂統領（架構師）+ 精神同步率強化 | 持續 | 有護盾時在 1.5 單位距離產生輪鋸繞角色旋轉（滿盾 6 個，2 秒一圈），撞敵消耗 2% 護盾 |
| 完美像素審判 | 疾光狙擊（超級導演）+ 視網膜增強模組 | 3 秒 | 繪製井字線，4 個焦點隨機順序爆炸（1 秒內完成） |
| 爆發的影視特效 | 疾光狙擊（超級導演）+ AI 賦能強化 | 3.5 秒 | 發射 20 枚追蹤導彈，隨機方向飛出 2 單位後直衝最近敵人 |
| 幻影迭代模式 | 咒言幻象（遊戲先知）+ AI 賦能強化 | 15 秒 | 召喚半透明紫色影分身，每 0.5 秒施放隨機進階技能，怪物優先攻擊分身，本尊與分身間每 0.5 秒彈射射線 |
...
| 技術美術大神 | 圓形範圍 + 攻擊強化 | 全能開發者天降神兵，精準打擊並癱瘓敵人（除錯中） |
| 絕對邏輯防禦 | 護盾技能 + 移速強化 | 架構師的防禦邏輯化為實體輪鋸，護盾越強輪鋸越多 |
| 完美像素審判 | 光束攻擊 + 暴擊強化 | 導演的精準構圖，井字線定位爆破目標 |
| 爆發的影視特效 | 光束攻擊 + 攻擊強化 | 導演的華麗特效，追蹤導彈群飛射擊 |
| 幻影迭代模式 | 圓形範圍 + 攻擊強化 | 先知的分身術，AI 複製體協同作戰，彈射射線連結本尊與分身 |
