...
## v0.9.22a - 遊戲模式系統 + 經驗水晶掉落

### 遊戲模式系統（新增）
- **難度選擇**：遊戲開始前選擇難度模式
- **三種模式**：
  | 模式 | 怪物倍率 | 經驗倍率 | 速度倍率 | 血量倍率 | 攻擊力倍率 |
  |------|----------|----------|----------|----------|------------|
  | 日常 | x1.0 | x1.0 | x0.8 | x1.0 | x1.0 |
  | 噩夢 | x2.0 | x2.0 | x1.2 | x2.0 | x1.5 |
  | 煉獄 | x3.0 | x3.0 | x1.5 | x2.5 | x2.0 |
- **UI 設計**：使用技能卡樣式，hover 彈起，點擊確認
- **遊戲流程**：模式選擇 → 技能選擇 → 開始遊戲
...
- **物件池**：上限 300，閾值 200 開始合併遠處水晶
- **合併機制**：超過閾值時刪除最遠水晶，經驗累加到下一個掉落
...
### 怪物調整
- 史萊姆基礎移動速度：1.0（標準速度）
- 速度倍率：日常 x0.8、噩夢 x1.2、煉獄 x1.5
- 血量倍率：日常 x1.0、噩夢 x2.0、煉獄 x2.5
- 攻擊力倍率：日常 x1.0、噩夢 x1.5、煉獄 x2.0
...
### 火花顏色常數化
所有技能火花顏色統一使用 `SparkColors`（SkillSystem.ts）管理：

| 常數名稱 | 普通 | 爆擊 | 技能 |
|---------|------|------|------|
| `SOUL_RENDER` | 0x6699ff | 0x88bbff | 靈魂渲染、三向衝擊波 |
| `VFX_SNIPE` | 0x66ff66 | 0x88ff88 | 疾光狙擊、連鎖射線 |
| `SAWBLADE` | 0xffcc00 | 0xffee44 | 絕對邏輯輪鋸、八角爆盾 |
| `TECH_ARTIST` | 0x9966ff | 0xbb88ff | 技術美術大神 |
| `SOUL_SLASH` | 0xff4466 | 0xff6688 | 次元向量疾劃 |
| `CELLULOID` | 0x4488ff | 0x66aaff | 燃燒賽璐珞 |

### 技術調整
- **SparkColors**：火花顏色常數從 MainScene 移至 SkillSystem.ts（-14 行 / +22 行）
- **getCritChance**：新增公開方法取得總爆擊率（供自訂爆擊倍率技能使用）

---

## v0.9.15a - 絕對邏輯防禦重構與怪物平衡

### 次元向量疾劃調整
- **暴擊傷害**：1.5 倍 → 3 倍（獨立計算，不受通用暴擊倍率影響）
- **暴擊特效**：暴擊時火花顏色更亮

### 絕對邏輯防禦重構
- **輪鋸數量**：每 10 級 +1 → 每 5 級 +1，上限 6 → 8
- **護盾耗損**：2% → 1%
- **飛出機制**：護盾覆蓋時輪鋸朝最近敵人飛去
- **彈跳追蹤**：擊中後搜尋前進方向 ±45°（90度扇形）內最近敵人繼續飛行
- **飛行速度**：每秒 15 單位，飛出 15 單位範圍才消失

### 怪物生成與強化
- **生成數量**：每次生成 6 隻
- **動態間隔**：間隔 = 3 秒 / 倍率（最短 100ms）
- **倍率公式**：180 / 場上怪物數量（怪物越少生越快）
- **強化等級**：70 級 → 60 級開始，每 5 級 +15%

---

## v0.9.14a - 幻影分身視覺強化與經驗平衡調整

### 經驗與菁英怪調整
- **經驗翻倍週期**：每 5 級 → 每 10 級翻倍（降低後期經驗膨脹）
- **菁英怪生成週期**：每 10 級 → 每 5 級生成一隻（5, 10, 15...）
- **血球大小**：3% → 5% 畫面高度（更容易看見）
